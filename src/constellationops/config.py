...
class Settings:
    ...
    def __post_init__(self) -> None:
        ...
        if self.offline_after_seconds < 1:
            raise ValueError("offline_after_seconds must be at least 1")
    ...
    def from_env(cls) -> Settings:
        return cls(
            udp_host=os.environ.get("UDP_HOST", _UDP_HOST),
            udp_port=_parse_int_env("UDP_PORT", _UDP_PORT),
            queue_capacity=_parse_int_env("QUEUE_CAPACITY", _QUEUE_CAPACITY),
            recent_sequence_history=_parse_int_env(
                "RECENT_SEQUENCE_HISTORY", _RECENT_SEQUENCE_HISTORY
            ),
            stale_after_seconds=_parse_int_env(
                "STALE_AFTER_SECONDS", _STALE_AFTER_SECONDS
            ),
            offline_after_seconds=_parse_int_env(
                "OFFLINE_AFTER_SECONDS", _OFFLINE_AFTER_SECONDS
            ),
